...
func (mj *Struct) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	...
	buf.WriteString(`,"Field7":`)
	if mj.Field7 != nil {
		buf.WriteString(`"`)
		{
			enc := base64.NewEncoder(base64.StdEncoding, buf)
			enc.Write(reflect.Indirect(reflect.ValueOf(mj.Field7)).Bytes())
			enc.Close()
		}
		buf.WriteString(`"`)
	} else {
...
var ffj_key_Struct_Field7 = []byte("Field7")
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				...
				case 'F':
					...
					} else if bytes.Equal(ffj_key_Struct_Field7, kn) {
						currentKey = ffj_t_Struct_Field7
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.AsciiEqualFold(ffj_key_Struct_Field7, kn) {
					currentKey = ffj_t_Struct_Field7
					state = fflib.FFParse_want_colon
					goto mainparse
				}
...
handle_Field7:
	...
	/* handler: uj.Field7 type=[]uint8 kind=slice quoted=false*/

	{
		...
		if tok == fflib.FFTok_null {
			...
			v := reflect.ValueOf(&uj.Field7).Elem()
			v.SetBytes(b[0:n])
