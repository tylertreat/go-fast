...
func BenchmarkMultithreaded16Thread9010Trie(b *testing.B) {
	numItems := 1000
	numThreads := 16
	benchmark9010(b, numItems, numThreads, func(items [][]string) matching.Matcher {
		return matching.NewTrieMatcher()
	})
}
...
func benchmark9010(b *testing.B, numItems, numThreads int, factory func([][]string) matching.Matcher) {
	itemsToInsert := make([][]string, 0, numThreads)
	for i := 0; i < numThreads; i++ {
		items := make([]string, 0, numItems)
		for j := 0; j < numItems; j++ {
